...
| 4. Stale version example in wasm docs        | **FIXED**   | The example output for the `version()` function in `src/wasm_api.rs` has been updated to `0.0.1` to match the version in `Cargo.toml`.                                                          |
| 5. `GfError::EmptyDrawPile` unused           | **FIXED**   | The `GfError::EmptyDrawPile` variant has been removed from `src/error/mod.rs`, eliminating the dead public API.                                                                                |
