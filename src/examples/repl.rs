...
fn human_turn(
    ...
    let valid_str = valid.iter().map(|c| c.to_string()).collect::<Vec<_>>().join("/");
...
fn bot_turn(
    ...
    if let PlayerAction::Ask { target, rank } = &action {
        println!("\n  {} asks {} for {}s.", names[cp], names[*target], rank.index);
    }
...
fn print_event(event: &GameEvent, names: &[String]) {
    match event {
        ...
        GameEvent::Drew { player, matched } => {
            if *matched {
                println!(
                    "  -> {} drew a match!  Gets to ask again.",
                    names[*player]
                );
            } else {
                println!("  -> {} drew (no match).  Next player's turn.", names[*player]);
            }
        }
...
fn rank_groups(hand: &BasicPile) -> Vec<(char, usize)> {
    ...
    groups.into_iter().map(|(ch, _, count)| (ch, count)).collect()
}
